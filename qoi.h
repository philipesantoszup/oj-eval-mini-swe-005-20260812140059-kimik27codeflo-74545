...
inline int QoiSignedDiff(uint8_t cur, uint8_t prev) {
...
bool QoiEncode(uint32_t width, uint32_t height, uint8_t channels, uint8_t colorspace) {
    ...
    if (colorspace != 0 && colorspace != 1) return false;
    ...
    for (uint64_t i = 0; i < px_num; ++i) {
        ...
        if (history[idx][0] == r && history[idx][1] == g && history[idx][2] == b && history[idx][3] == a) {
            ...
            int dr = QoiSignedDiff(r, pre_r);
            int dg = QoiSignedDiff(g, pre_g);
            int db = QoiSignedDiff(b, pre_b);
            ...
            if (a == pre_a && dr >= -2 && dr <= 1 && dg >= -2 && dg <= 1 && db >= -2 && db <= 1) {
                ...
                if (dr_dg >= -8 && dr_dg <= 7 && db_dg >= -8 && db_dg <= 7) {
                    ...
                    QoiWriteU8(static_cast<uint8_t>((dr_dg + 8) << 4) | static_cast<uint8_t>(db_dg + 8));
                } else {
...
bool QoiDecode(uint32_t &width, uint32_t &height, uint8_t &channels, uint8_t &colorspace) {
    ...
    for (uint64_t i = 0; i < px_num; ++i) {
        ...
        if (b1 == QOI_OP_RGB_TAG) {
            ...
            int run = (b1 & 0x3f) + 1;
            if (run > 62 || static_cast<uint64_t>(run) > px_num - i) return false;
            for (int j = 0; j < run; ++j) {
                QoiWriteU8(r);
                QoiWriteU8(g);
                QoiWriteU8(b);
